...
pub fn read(self: EventFd) !void {
    ...
    if (@as(i64, @bitCast(std.os.linux.read(self.fd, &buf, buf.len))) < 0) return error.ReadError;
}
pub fn write(self: EventFd) !void {
    const msg: usize = 1;
    if (@as(i64, @bitCast(std.os.linux.write(self.fd, @ptrCast(&msg), @sizeOf(usize)))) < 0)
